    ...
    <div class='container'>
        <div class='form-content'>
            <el-form :inline='true' :model='computerRecordForm' class='demo-form-inline'>
                ...
                <el-form-item label='学生姓名'>
                    <el-input v-model='computerRecordForm.student' placeholder='请输入上机记录号码'></el-input>
                </el-form-item>
        ...
        <div class='table-content'>
            <el-table v-loading='tableLoading' ref='multipleTable' :data='computerRecordData' tooltip-effect='dark'
                ...
                <el-table-column prop='student' label='学生' width='160'>
                    <template slot-scope='scope'>
                        <span>{{ scope.row.studentObject.name }}</span>
                    </template>
                </el-table-column>
...
<script>
import { deleteComputerRecordById, getComputerRecordByPage } from '@/api/basic/computerRecord';
...
export default {
    ...
    methods: {
         ...
         */
        initComputerRecord: async function() {
        ...
        handleSelectionChange(val) {
            ...
            this.multipleSelectionComputerRecord = selected;
        },
        ...
        handleDelete(type, id) {
            ...
            if (type === '批量' && id === null) {
                // 当前没有选中数据
                if (this.multipleSelectionComputerRecord.length === 0) {
                    return this.$message({ type: 'error', message: '请选择删除对象' });
                }
            }
            this.$confirm('此操作将永久删除选中上机记录信息, 是否继续?', '确定删除', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                // 批量删除或单条数据删除，走同一个后端接口
                deleteComputerRecordById(type === '批量' ? this.multipleSelectionComputerRecord.join(',') : id).then(res => {
                    if (res.code === 200) {
                        this.$message({
                            type: 'success',
                            message: '删除' + type === '批量' ? this.multipleSelectionComputerRecord.length + '条数据成功！' : '成功！'
                        });
                        // 重新查询更新数据
                        this.querySubmit();
                    } else {
        ...
        handleEditOrAdd(row, type) {
            ...
            if (this.actionType === '新增') {
                this.selectedComputerRecord = {
                    student: { id: 0, name: '' },
                    computer: { id: 0, number: '' },
                    startTime: '',
                    endTime: ''
                };
            } else {
                this.selectedComputerRecord = row;
            }
            this.showDialog = true;
            // 对话框展开
            this.$nextTick(() => {
                this.$refs['computerRecordForm'].showDialog = true;
            });
        },
        /**
         * 关闭对话框
