<template>
    <el-dialog :title="actionType + '电脑配置'" :visible.sync='showDialog' @close='closeDialog(0)'>
        <el-form :model='vehicleForm' :rules='rules' ref='vehicleForm' label-width='100px' class='demo-vehicleForm'>

            <el-form-item label='电脑配置编号' prop='number'>
                <el-input v-model='vehicleForm.hardware' placeholder='请输入硬件配置'></el-input>
            </el-form-item>
            <el-form-item label='电脑配置号码' prop='licensePlate'>
                <el-input v-model='vehicleForm.software' placeholder='请输入软件'></el-input>
            </el-form-item>
...
export default {
    ...
    data() {
        ...
        return {
            showDialog: false,
            // 表单数据
            vehicleForm: { ...this.selectedComputerConfiguration },
            // 表单校验规则
            rules: {
                hardware: [
